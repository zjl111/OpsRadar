...
def seed_builtin_data(db: Session) -> None:
    ...
    analysis_rules = [
        (
            ...
            "SSH 认证失败：资源中配置的用户名、密码或私钥未通过目标主机认证，或目标主机不允许该用户通过 SSH 登录。",
            "会导致该主机上的 OS、安全基线和容器服务巡检无法执行，影响环境健康判断。",
            "检查资源配置中的用户名、密码或私钥是否正确，确认目标主机允许该用户通过 SSH 登录；必要时重新录入凭据。",
            ["在资源列表点击测试确认 SSH 连通性。", "检查账号是否被锁定、过期，或是否禁止 root 登录。", "如果使用私钥，确认私钥内容、passphrase 和目标主机 authorized_keys。", "检查 sshd_config 中 PasswordAuthentication、PubkeyAuthentication、PermitRootLogin、AllowUsers 等策略。"],
            "资源测试成功后重新启动巡检任务。",
            "high",
        ),
        (
            ...
            "fail",
            "",
            "主机内存占用超过巡检阈值，可能是业务进程、容器、缓存或系统 page cache 持续占用导致。",
            "可能导致服务响应变慢、进程被 OOM Kill，或影响同机容器和数据库稳定性。",
            "定位内存占用最高的进程/容器，检查近期发布、缓存增长和 OOM 记录；必要时释放异常进程或扩容。",
            ["查看 free -h、top/ps 内存排序。", "检查 dmesg 或 journalctl 中是否存在 OOM。", "如果运行 Docker，查看 docker stats 中内存占用最高的容器。", "处理后重新执行内存巡检项。"],
            "内存占用回落到阈值内，且无新的 OOM 记录。",
            "medium",
        ),
        (
            "arl_os_disk_inode",
            ...
            "fail",
            "",
            "磁盘空间或 inode 使用率超过阈值，可能是日志、缓存、临时文件、容器镜像或小文件堆积导致。",
            "可能导致日志写入失败、服务启动失败、数据库写入异常或容器调度异常。",
            "清理无用日志、临时文件、旧镜像和异常增长目录；如果是业务数据增长，应评估扩容。",
            ["执行 df -h 和 df -i 确认空间与 inode。", "使用 du -xhd1 定位增长最快目录。", "清理过期日志、缓存、无用镜像或临时文件。", "重新执行磁盘/inode 巡检项验证。"],
            "磁盘和 inode 使用率回落到阈值内，相关服务写入正常。",
            "medium",
        ),
        (
            "arl_os_tcp_connections",
            "主机 TCP 连接状态异常",
            "os",
            "host",
            "TCP",
            "fail",
            "",
            "主机 TCP 连接状态超过巡检阈值，可能存在连接堆积、TIME_WAIT 过多、端口耗尽或后端服务响应慢。",
            "可能影响 API、数据库、中间件或网关连接稳定性。",
            "分析 ss/netstat 连接状态分布，确认是否有异常客户端、连接泄漏、超时或服务端处理慢。",
            ["执行 ss -s 和 ss -tan state time-wait/established 统计连接。", "按远端 IP、端口聚合连接来源。", "检查应用连接池、超时配置和网关转发状态。", "重新执行 TCP 连接状态巡检项验证。"],
            "异常连接状态回落，核心端口连接建立和释放正常。",
            "medium",
        ),
        (
            "arl_redis_memory",
