...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("service_resource_id", sa.String(length=40), nullable=True),
        ...
        sa.ForeignKeyConstraint(["service_resource_id"], ["resources.id"], ondelete="SET NULL"),
    ...
    op.create_index(op.f("ix_discovered_services_service_resource_id"), "discovered_services", ["service_resource_id"], unique=False)


def downgrade() -> None:
    op.drop_index(op.f("ix_discovered_services_service_resource_id"), table_name="discovered_services")
